...
class CachedFileHandle {
	...
	async write(offset, buffer){
		...
		if(fileChunkOffset == lastFileChunkOffset){
			...
			const writePromises = [];
			// We need to flush caches that are only getting partially overwritten
			if(this._chunks.has(fileChunkOffset) && fileChunkOffset != offset){
				writePromises.push(this.fd.write(
					this._chunks.get(fileChunkOffset),
					0,
					offset - fileChunkOffset
				));
			}
			if(
				this._chunks.has(lastFileChunkOffset) &&
				(lastFileChunkOffset + this.chunkSize) != (offset + buffer.length)
			){
				const chunk = this._chunks.get(lastFileChunkOffset);
				const chunkWriteStart = (offset + buffer.length) - lastFileChunkOffset;
				writePromises.push(this.fd.write(
					chunk,
					chunkWriteStart,
					chunk.length - chunkWriteStart,
			...
			writePromises.push(this.fd.write(buffer, 0, buffer.length, offset));
