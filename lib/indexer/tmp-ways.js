...
class TempWayGeoFinder {
	...
	async prepareWayGeometries(wayIDs){
		/**@type {Array<number>} */
		const firstWayArray = [...this.offsetMap.keys()];
		let wayIndex = 0;
		this._wayIDs = wayIDs;
		/**@type {number} */
		let firstWayIndex = bounds.le(firstWayArray, wayIDs[0]);
		if(firstWayIndex == -1){
		...
		while(this._geometries.length < this._wayIDs.length){
			if(firstWayIndex >= firstWayArray.length){
				this._geometries.push(null);
				continue;
			}
			const geoFileOffset = this.offsetMap.get(firstWayArray[firstWayIndex]);
