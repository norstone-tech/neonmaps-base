	...
	async prepareWayGeometries(wayIDs){
		...
		while(this._geometries.length < this._wayIDs.length){
			...
			if(wayMapOffsetIndex == (this.wayOffsetFinder.length - 1)){
				...
				const nextIndex = await this.wayOffsetFinder.le(wayIDs[wayIndex], wayMapOffsetIndex + 1);
				if(nextIndex == -1 || nextIndex == wayMapOffsetIndex){
					wayMapOffsetIndex += 1;
				}else{
					wayMapOffsetIndex = nextIndex;
				}
			}
		}
	}
