...
class MapReaderBase {
	...
	async init(){
		try{
			...
			this.fileSize = (await this.fd.stat()).size;
			/**@type {OSMHeader} */
	...
	/**
	 * @returns {Generator<Promise<number>, undefined, void>}
	 */
	*offsets(){
		let offset = this.mapHeader._byte_size;
	...
	/**
	 * @returns {Generator<Promise<OSMDecodeResult>, undefined, void>}
	 */
	*mapSegments(){
		const a = this.offsets();
		/**@type {Promise<number>} */
		let offsetPromise;
		for(offsetPromise of this.offsets()){
			yield offsetPromise.then(num => this.readDecodedMapSegment(num));
		}
	}
	/**
	 * @async
	...
	for(let i = 0; i < rawData.primitivegroup.length; i += 1){
		...
		if(rawGroup.dense){
			...
			let lastUnpackedNode = {
				type: "node",
				id: denseNodes.id[0],
				lat: Number((0.000000001 * (latOffset + (posGranularity * lastRawLat))).toFixed(9)),
				lon: Number((0.000000001 * (lonOffset + (posGranularity * lastRawLon))).toFixed(9)),
		...
		for(let ii = 0; ii < rawGroup.nodes.length; ii += 1){
			...
			const node = {
				id: rawNode.id,
				type: "node",
				lat: Number((0.000000001 * (latOffset + (posGranularity * rawNode.lat))).toFixed(9)),
				lon: Number((0.000000001 * (lonOffset + (posGranularity * rawNode.lon))).toFixed(9)),
				tags: new MapWithToJSON()
			};
