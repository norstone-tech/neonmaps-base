 ...
 * @property {Array<"NODE" | "WAY" | "RELATION">} types
 */
/**
 ...
 */
MapReader.decodeRawData = function(rawData){
	/**@type {Array<OSMNode>} */
	const nodes = [];
	/**@type {Array<OSMWay>} */
	const ways = [];
	...
	for(let i = 0; i < rawData.primitivegroup.length; i += 1){
		...
		if(rawGroup.dense){
			...
			console.log("FIRST NODE ID:", lastUnpackedNode.id)
			...
			const relation = {
				...
				members: rawRelation.memids.map((memid, iii) => {
					return {
						...
						type: rawRelation.types[iii].toLowerCase()
					}
				}),
				tags: new MapWithToJSON()
			};
