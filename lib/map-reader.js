...
class MapReader extends MapReaderBase {
	...
	constructor(filePath, rawCacheAmount = 0, decodedCacheAmount = 0, diskCacheSize = 0, checksum){
		...
		this.eifh = new CachedFileHandle(this.elemIndexFilePath);
	}
	async init(){
		await super.init();
		try{
			await this.eifh.open();
			const indexFileHeader = await this.eifh.read(0, ELEMENT_INDEX_HEADER_SIZE);
			...
			const nodeIndexOffset = (
				await this.eifh.read(fileOffset, ELEMENT_INDEX_OFFSET_SIZE)
			).readUIntLE(0, ELEMENT_INDEX_OFFSET_SIZE);
			fileOffset += ELEMENT_INDEX_OFFSET_SIZE;
			
			const wayIndexOffset = (
				await this.eifh.read(fileOffset, ELEMENT_INDEX_OFFSET_SIZE)
			).readUIntLE(0, ELEMENT_INDEX_OFFSET_SIZE);
			fileOffset += ELEMENT_INDEX_OFFSET_SIZE;
			
			const relationIndexOffset = (
				await this.eifh.read(fileOffset, ELEMENT_INDEX_OFFSET_SIZE)
			).readUIntLE(0, ELEMENT_INDEX_OFFSET_SIZE);
			fileOffset += ELEMENT_INDEX_OFFSET_SIZE;
