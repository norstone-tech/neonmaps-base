...
class MapReader extends MapReaderBase {
	...
	constructor(
		...
		this.elemIndex = new OSMElementIDIndexReader(this.filePath, this.checksum);
		this.parentIndex = new OSMElementParentIndexReader(this.filePath, this.checksum);
	...
	async getWayGeometry(wayID){
		...
		const result = rawGeometry.lat.map((lat, i) => {
			return [
				Number((rawGeometry.lon[i] / 1000000000).toFixed(9)),
				Number((lat / 1000000000).toFixed(9))
			];
		});
	...
	async getRelationGeometry(relID){
		...
		return rawGeometry.map(rawGeo => {
			const points = rawGeo.lat.map((lat, i) => {
				return [
					Number((rawGeo.lon[i] / 1000000000).toFixed(9)),
					Number((lat / 1000000000).toFixed(9))
				];
			});
			if(rawGeo.closed){
	...
	async getNodeGeoJSON(node){
		...
		return turf.point([node.lon, node.lat], node.tags.toJSON(), {id: "node/" + node.id});
	}
	/**
	 * @async
	...
	async getWayGeoJSON(way){
		...
		if(wayGeo[0] == wayGeo[wayGeo.length - 1]){
			return turf.polygon([wayGeo], way.tags.toJSON(), {id: "way/" + way.id});
		}else{
			return turf.lineString(wayGeo, way.tags.toJSON(), {id: "way/" + way.id});
		}
	}
	/**
	 * @async
		...
		switch(relation.tags.get("type")){
			...
			case "boundary":
				...
				if(polygons.length == 1){
					return turf.polygon(polygons[0], relation.tags.toJSON(), {id: "relation/" + relation.id});
				}else{
					return turf.multiPolygon(polygons, relation.tags.toJSON(), {id: "relation/" + relation.id});
				}
			case "route":
				if(relGeo.length == 1){
					return turf.lineString(relGeo[0].points, relation.tags.toJSON(), {id: "relation/" + relation.id});
				}else{
					return turf.multiLineString(
						relGeo.map(({points}) => points),
